...
def predecir_tiempos_streamlit(maquina, grouped, codcent_nombre):
    ...
    for codcent, grupo in grouped:
        ...
        if existe.any():  # Si existe un valor exacto en los registros
            ...
            st.write(f"Nº de horas imputadas en el centro  {int(codcent)} - {nombre_codcent}: {valores_reales} horas")
            ...
            st.write(f"Predicción de horas para el centro {int(codcent)} - {nombre_codcent}: {rf_predictions} horas")
